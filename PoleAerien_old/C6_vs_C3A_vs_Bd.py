...
class C6_vs_C3A_vs_Bd:
    ...
    def LectureFichiersExcelsC6(self, df, fichier_c6):
        ...
        try:
            ...
            print(f"Erreur avec ce fichier {fichier_c6}\nPrécision : {e}")
    ...
    def liste_poteau_c3a_excel(self, fichier_c3a):
        ...
        try:
            ...
            print(f"FICHIER : {fichier_c3a}\nLettre : {lettre}")
            df4 = pd.DataFrame({})
            df4_rempl = pd.DataFrame({})

        except Exception as e:
            print(f"Erreur avec ce fichier {fichier_c3a}\nPrécision : {e}")
    ...
    def liste_poteau_etudes(self, table_poteau, table_decoupage, champs_dcp, valeur_chp_dcp):
        ...
        decoupage = QgsProject.instance().mapLayersByName(table_decoupage)[0]
        ...
        requete = QgsExpression(f"{champs_dcp} LIKE '{valeur_chp_dcp}'")
        ...
        for feat_dcp in decoupage.getFeatures(request_cap_ft):
            # etudes = feat_dcp[colonne_cap_ft]
            for feat_pot in infra_pt_pot.getFeatures(request):
                if feat_dcp.geometry().contains(feat_pot.geometry()):
    ...
    def liste_poteau_c3a_qgis(self, table_cmd, table_decoupage, champs_decoupage, valeur_champs_dcp):
        """Récupérer les données liées à la base de données"""
        cmd = QgsProject.instance().mapLayersByName(table_cmd)[0]
        decoupage = QgsProject.instance().mapLayersByName(table_decoupage)[0]
        ...
        requete_dcp = QgsExpression(f"{champs_decoupage} LIKE '{valeur_champs_dcp}'")
        ...
        for feat_dcp in decoupage.getFeatures(request_dcp):
            ...
            for feat_cmd in cmd.getFeatures(request_cmd):
                if feat_dcp.geometry().contains(feat_cmd.geometry()):
        ...
        if numeroAppui:
            df = pd.DataFrame({'N° appui': numeroAppui, "Nature des travaux": nature_des_travaux,
                               'inf_num (C3A)': num_a_b, 'ETAT (C3A)': remp_perc_a_b}, dtype="float64")

            df_rempl = pd.DataFrame({'N° appui': numeroAppui_rempl, 'inf_num (C3A)': num_a_b_rempl,
                                     'ETAT (C3A)': remp_perc_a_b_rempl}, dtype="float64")
...
#                 'Composite (R1) \n8 m\n48 Kg', 'Date 1ère livraison', 'Date 2ème livraison']
        ...
        # fichier_c7 = f"C:/Users/asoumare/Documents/Teletravail/Developpement/CAP_COMAC/D17-10/F56971281122_C7.xlsx"
        ...
        try:
            ...
            print(f"FICHIER : {fichier_c7}\nLettre : {lettre}")
            df4 = pd.DataFrame({})

        except Exception as e:
            print(f"Erreur avec ce fichier {fichier_c7}\nPrécision : {e}")
